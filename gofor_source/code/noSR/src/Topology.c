...
Topology_t* Topology_load_from_file_with_labels(const char* filename, int precision, char biDir, LabelTable_t* labels)
{
...
Topology_t* Topology_load_from_file_with_ids(const char* filename, int precision, char biDir)
{
    int src, dst;
    my_m2 m2;
    char line[1024];
...
void dikjstra_best_m2(Edge_t**** succOutGraph, Edge_t**** predOutGraph, Llist_t** ingraph, 
                int root, my_m1** m1dists, my_m2** m2dists, bool** has_ecmp, int nbNodes)
{
    BinHeap_t bp;
    ...
    for (int currNode = root, nbSeen = 0 ; nbSeen < nbNodes * nbNodes && currNode != -1 ; nbSeen++) {
        for (Llist_t* neighbor = ingraph[currNode] ; neighbor != NULL ; neighbor = neighbor->next) {
            ...
            if ((pathM2 = neighbor->infos.m2 + (*m2dists)[currNode]) == (*m2dists)[neighbor->infos.edgeDst]) {
                (*has_ecmp)[neighbor->infos.edgeDst] = 1;
                // printf("ECMP between %d and %d\n", root, neighbor->infos.edgeDst);
                ...
                (*has_ecmp)[neighbor->infos.edgeDst] = (*has_ecmp)[currNode];
                // if ((*has_ecmp)[neighbor->infos.edgeDst]) {
                //     printf("ECMP between %d and %d\n", root, neighbor->infos.edgeDst);
                // }
                parents[neighbor->infos.edgeDst][0] = 1;
                parents[neighbor->infos.edgeDst][1] = currNode;
